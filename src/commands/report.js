const { EmbedBuilder, ActionRowBuilder, ButtonBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, Colors } = require('discord.js');
...
module.exports = {
  ...
  async execute(message) {
    try {
      ...
      if (!supportGuild) {
        return message.reply('O servidor de suporte principal não foi encontrado.');
      }

      const forumChannel = supportGuild.channels.cache.get(forumChannelId);

      if (!forumChannel || forumChannel.type !== 15) {
        return message.reply('O canal de fórum não foi encontrado ou não é um canal de fórum.');
      }
      ...
      message.client.once('interactionCreate', async (interaction) => {
        if (!interaction.isButton() || interaction.customId !== 'open-report-modal') return;
      ...
      message.client.once('interactionCreate', async (interaction) => {
        ...
        if (interaction.user.id !== message.author.id) {
          return interaction.reply({ content: 'Você não pode responder a esse formulário.', ephemeral: true });
        }
